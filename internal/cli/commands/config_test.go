...
func TestConfigInitCreatesFile(t *testing.T) {
	tempDir := t.TempDir()
	t.Setenv("CURSOR_RULES_CONFIG_DIR", tempDir)
	t.Setenv("CURSOR_RULES_PACKAGE_DIR", tempDir)
	t.Setenv("PATH", "")
	...
	content := string(data)
	if !strings.Contains(content, "packageDir: "+tempDir) {
		t.Fatalf("expected packageDir entry, got:\n%s", content)
	}
...
func TestConfigInitSupportsSeparateConfigAndPackageDirs(t *testing.T) {
	configDir := t.TempDir()
	...
	data, err := os.ReadFile(cfgPath)
	if err != nil {
		t.Fatalf("config file not created in config dir: %v", err)
	}
	if !strings.Contains(string(data), "packageDir: "+packageDir) {
		t.Fatalf("expected packageDir entry to use package dir env, got:\n%s", string(data))
	}
}
